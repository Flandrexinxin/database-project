    ...
    if request.method == 'POST':
        ...
        if filename == '':
            ...
            if file_ext != '.csv':
                 error = "仅支持csv文件的上传"
            if error is None:
                uploaded_file.save(os.path.join('project/static/FILES', filename))
                file_path = os.path.join('project/static/FILES',filename)
                ...
                if error is None:
                    ...
                    with open('project/static/FILES/new_account.csv','w') as new:
...
def DBdownload():
    user_name = session['user_name']
    # return send_from_directory(r"C:\\Users\\Lenovo\\gitee\\database-project\\our_project\\project/static/FILES","new_account.csv")
    from flask import send_file 
    download_path = '../project/static/FILES/new_account.csv' 
