    ...
    if request.method == 'POST':
        ...
        if error is None:
            ...
            if(length==0):
                ...
                with open('project/static/FILES/inquire_result.csv','w') as result_file:
    ...
    if request.method == 'POST':
        ...
        if error is None:
            ...
            if(length==0):
                ...
                header = ['身份证号','姓名','联系电话','性别','出生日期','居住地','进入居住地时间','离开居住地时间']
                with open('peoject/static/FILES/inquire_result.csv','w') as result_file:
                    writer = csv.writer(result_file)
                    writer.writerow(header)
                    for i in range(length):
...
def DBdownload():
    if request.method == 'POST':
        ...
        download_path = '../peoject/static/FILES/inquire_result.csv' 
